 ...
 */
 ...
 export default function RemoveTeamModal({ teamDivision, setRemoveModal, listTeamDivisionsFunc }) {
   ...
   const deleteTeamDivisionFunc = async (e) => {
     ...
     <div tabIndex="-1" class="z-[200] fixed top-[10rem] right-0 left-[0] p-4 overflow-x-hidden overflow-y-auto w-[32rem] mx-auto">
       <div class="relative w-full h-full max-w-md mx-auto w-[25rem]">
           <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
               <button onClick={(e) => setRemoveModal(false)} type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-hide="popup-modal">
               ...
               <div class="p-6 pr-[3.6rem] text-center min-w-[30rem]">
                   ...
                   <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure you want to REMOVE<br/><b><i> {teamDivision.team.name}</i></b> <br/>from Division <b>{teamDivision.division.name}?</b></h3>
                   <button onClick={() => deleteTeamDivisionFunc()} data-modal-hide="popup-modal" type="button" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                       Yes, I'm sure
                   </button>
     ...
     <div onClick={(e) => setRemoveModal(false)} class='z-[150] bg-gray-500 opacity-50 fixed top-0 left-0 w-[100%] h-[100%]' />
     </>
     )
 }