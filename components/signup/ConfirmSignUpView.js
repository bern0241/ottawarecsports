        ...
        <div className='max-w-[33rem] py-[4rem] px-[3rem] mx-auto translate-y-[-20px]'>
                ...
                <div className='mt-[1.5rem] w-[20rem]'>
                    <div className='mb-7'>
                        ...
                        id='confirmationCode' type='text' state={confirmationCode} setState={setConfirmationCode} />
                    </div>

                    {message !== null && (<p id="message-notice" className={`ml-1 text-[.87rem] ${message.status === 'error' ? 'text-red-600': 'text-green-500' } relative bottom-2`}><span class="font-medium"></span> {message.message}</p>) }

                    <button onClick={() => confirmSignUp()} className='mt-5 p-[10px] w-full bg-[#007916] text-white rounded-sm'>Confirm Sign Up</button>
                    <p className='py-3 text-right'></p>

                </div>

                <p className='text-center underline cursor-pointer text-[.92rem] text-[#00440c]' onClick={resendConfirmationCode}>Resend Verification Code</p>
