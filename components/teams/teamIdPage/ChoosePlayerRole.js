...
export default function ChoosePlayerRole({ clickStopPropagationFunc, setChangeRoleModal, setNewRole, currentRole}) {
  ...
  return (
    ...
    <button onClick={(e) => {
        ...
        e.preventDefault();
        setOpen(!open);
        }} id="dropdownDefaultButton" data-dropdown-toggle="dropdown" className="border-gray-400 border-[1px] text-black bg-white hover:bg-gray-100 focus:ring-[2px] focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-4 py-2.5 text-center inline-flex items-center mx-3" type="button">{currentRole} <svg className="w-4 h-4 ml-2" aria-hidden="true" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg></button>
